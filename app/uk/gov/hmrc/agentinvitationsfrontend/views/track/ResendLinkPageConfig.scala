...
import uk.gov.hmrc.agentinvitationsfrontend.models.Services.{HMRCMTDVAT, HMRCPIR, TAXABLETRUST}
...
case class ResendLinkPageConfig(
  ...
  val step1Instructions: Option[String] = if (clientType == "personal") {
    ...
    else if (service == TAXABLETRUST) Some(Messages("invitation-sent.step1.business.trust"))
    else None
  } else None
}
