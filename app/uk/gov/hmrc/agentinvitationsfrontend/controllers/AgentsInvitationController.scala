...
class AgentsInvitationController @Inject()(
  ...
  private[controllers] def createInvitation[T <: TaxIdentifier](arn: Arn, fti: FastTrackInvitation[T])(
    ...
    invitationsService
      .createInvitation(arn, fti.service, fti.clientIdentifierType, fti.clientIdentifier, fti.knownFact)
      .map(invitation => {
          ...
          aggregate.service match {
            case Some(HMRCMTDVAT) =>
              fastTrackCache
                ...
                .map(_ => Forbidden(not_matched(Services.messageKeyForVAT)))
            case Some(HMRCMTDIT) =>
  ...
  val agentInvitationServiceForm: Form[UserInputNinoAndPostcode] = {
    Form(
      mapping(
        ...
        "postcode"         -> optional(text))({ (service, _, _) =>
