...
class AgentInvitationKfcExtractorSpec extends UnitSpec {
  ...
  private val featureFlagsKFAllOff = new FeatureFlags(
    showHmrcMtdIt = true,
    showPersonalIncome = true,
    showHmrcMtdVat = true,
    showKfcMtdIt = false,
  ...
  "The ClientForMtdItWithFlagOn extractor" should {
    ...
    "Return None for HMRC-MTD-IT service when KFC feature flag is off" in {
      val args = (UserInputNinoAndPostcode(personal, serviceITSA, Some(mtdItId.value), None), featureFlagsKFAllOff)
  ...
  "The ClientForPirWithFlagOn extractor" should {
    ...
    "Return None for PERSONAL-INCOME-RECORD service when KFC feature flag is off" in {
      val args = (UserInputNinoAndPostcode(personal, servicePIR, None, None), featureFlagsKFAllOff)
  ...
  "The ClientForVatWithFlagOn extractor" should {
    ...
    "Return None for HMRC-MTD-VAT service when KFC feature flag is off" in {
      val args = (UserInputVrnAndRegDate(business, serviceVAT, Some(vrn.value), None), featureFlagsKFAllOff)
  ...
  "The ClientWithItsaOrPirFlagOff extractor" should {
    "Return something for HMRC-MTD-IT service when details match and KFC feature flag is off" in {
      val args = (UserInputNinoAndPostcode(personal, serviceITSA, Some(mtdItId.value), None), featureFlagsKFAllOff)
      ClientWithItsaOrPirFlagOff.unapply(args) shouldBe Some(())
    }
    "Return something for PERSONAL-INCOME-RECORD service when details match and KFC feature flag is off" in {
      val args = (UserInputNinoAndPostcode(personal, servicePIR, Some(nino.value), None), featureFlagsKFAllOff)
      ClientWithItsaOrPirFlagOff.unapply(args) shouldBe Some(())
    }
    "Return something for PERSONAL-INCOME-RECORD service when details match and showPersonalIncome flag is off" in {
      val args = (
        UserInputNinoAndPostcode(personal, servicePIR, Some(nino.value), None),
        featureFlagsKFAllOff.copy(showKfcMtdIt = true))
      ClientWithItsaOrPirFlagOff.unapply(args) shouldBe Some(())
    }
    "Return None for HMRC-MTD-IT service when details don't match and KFC feature flag is off" in {
      val args = (UserInputNinoAndPostcode(personal, serviceITSA, None, None), featureFlagsKFAllOff)
  ...
  "The ClientWithVatFlagOff" should {
    ...
    "Return None for HMRC-MTD-VAT service when details don't match and KFC feature flag is off" in {
      val args = (UserInputVrnAndRegDate(business, serviceVAT, None, None), featureFlagsKFAllOff)
