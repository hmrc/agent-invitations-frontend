...
import uk.gov.hmrc.agentinvitationsfrontend.models.AgentInvitationNinoForm
...
class AgentInvitationServiceFormSpec extends UnitSpec{
  ...
  "ServiceForm" should {
    ...
    "return no errors when unbinding the form" in {
      val unboundFormITSA = agentInvitationServiceForm.mapping.unbind(AgentInvitationNinoForm(serviceITSA, Some(Nino("AE123456C")), None))
      unboundFormITSA("service") shouldBe serviceITSA

      val unboundFormAFI = agentInvitationServiceForm.mapping.unbind(AgentInvitationNinoForm(servicePIR, Some(Nino("AE123456C")), None))
      unboundFormAFI("service") shouldBe servicePIR

      val unboundFormVAT = agentInvitationServiceForm.mapping.unbind(AgentInvitationNinoForm(serviceVAT, Some(Vrn("101747696")), None))
