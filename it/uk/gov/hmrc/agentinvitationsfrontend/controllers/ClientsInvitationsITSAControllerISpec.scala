...
class ClientsInvitationsITSAControllerISpec extends BaseISpec {
  ...
  "GET /accept-tax-agent-invitation/consent/:invitationId (confirm terms page)" should {
    ...
    "show the invitation expired page when invitation has expired" in {
      givenInvitationExpired(arn, mtdItId.value, invitationIdITSA, serviceITSA, identifierITSA)
      givenGetAgencyNameClientStub(arn)
      val reqITSA = authorisedAsValidClientITSA(FakeRequest().withSession("agencyName" -> "My Agency"), mtdItId.value)
      val resultITSA = getConfirmTermsITSA(reqITSA)
    ...
    "show the request-cancelled page when the invitation has already been cancelled" in {
      givenInvitationCancelled(arn, mtdItId.value, invitationIdITSA, serviceITSA, identifierITSA)
      givenGetAgencyNameClientStub(arn)
      val reqITSA = authorisedAsValidClientITSA(FakeRequest().withSession("agencyName" -> "My Agency"), mtdItId.value)
      val resultITSA = getConfirmTermsITSA(reqITSA)
    ...
    "redirect to IV uplift journey if an authenticated user has a Confidence Level below 200" in {
      ...
      val request = FakeRequest("GET", routes.ClientsInvitationController.getConfirmTerms(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.getConfirmTerms(invitationIdITSA)(
        authenticatedClient(
          request,
      ...
      checkRedirectedToIVUplift(
        result,
        expectedCompletionUrl = routes.ClientsInvitationController.getConfirmTerms(invitationIdITSA).url,
  ...
  "POST /accept-tax-agent-invitation/consent/:invitationId (clicking confirm on the confirm terms page)" should {
    ...
    "redirect to /incorrect/ if authenticated user has HMRC-MTD-IT or HMRC-NI enrolment but with a different clientId" in {
      val reqITSA = authorisedAsValidClientITSA(FakeRequest(), mtdItId.value)
        .withFormUrlEncodedBody("confirmTerms" -> "true")
        .withSession("agencyName" -> "My Agency")
      givenGetInvitationReturnsNoPermission(mtdItId.value, invitationIdITSA, identifierITSA)
      givenAcceptInvitationReturnsNoPermission(mtdItId.value, invitationIdITSA, identifierITSA)
      givenGetAgencyNameClientStub(arn)
      val resultITSA = submitConfirmTermsITSA(reqITSA)
    ...
    "redirect to invitationAlreadyResponded when an invitation is returned that is already actioned" in {
      val reqITSA = authorisedAsValidClientITSA(FakeRequest(), mtdItId.value)
        .withFormUrlEncodedBody("confirmTerms" -> "true")
        .withSession("agencyName" -> "My Agency")
      givenGetInvitationReturnsAlreadyActioned(mtdItId.value, invitationIdITSA, identifierITSA)
    ...
    "redirect to /client/not-authorised if an authenticated has a Confidence Level below 200" in {
      givenUnauthorisedForInsufficientConfidenceLevel()
      val request = FakeRequest("POST", routes.ClientsInvitationController.submitConfirmTerms(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.submitConfirmTerms(invitationIdITSA)(
        authenticatedClient(
          request,
  ...
  "GET /accept-tax-agent-invitation/accepted/:invitationId (complete page)" should {
    ...
    "show the complete page for ITSA" in {
      givenInvitationExists(arn, mtdItId.value, invitationIdITSA, serviceITSA, identifierITSA, "Accepted")
      givenGetAgencyNameClientStub(arn)
    ...
    "redirect to IV uplift journey if an authenticated user has a Confidence Level below 200" in {
      givenUnauthorisedForInsufficientConfidenceLevel()
      val request = FakeRequest("GET", routes.ClientsInvitationController.getCompletePage(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.getCompletePage(invitationIdITSA)(
        authenticatedClient(
          request,
      ...
      checkRedirectedToIVUplift(
        result,
        expectedCompletionUrl = routes.ClientsInvitationController.getCompletePage(invitationIdITSA).url,
        expectedFailureUrl = routes.ClientsInvitationController.notAuthorised().url
      )
    }
  }
  ...
  "GET /accept-tax-agent-invitation/confirm-decline/:invitationId (confirm decline page)" should {
    ...
    "redirect to IV uplift journey if an authenticated user has a Confidence Level below 200" in {
      givenUnauthorisedForInsufficientConfidenceLevel()
      val request = FakeRequest("GET", routes.ClientsInvitationController.getConfirmDecline(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.getConfirmDecline(invitationIdITSA)(
        authenticatedClient(
          request,
      ...
      checkRedirectedToIVUplift(
        result,
        expectedCompletionUrl = routes.ClientsInvitationController.getConfirmDecline(invitationIdITSA).url,
        expectedFailureUrl = routes.ClientsInvitationController.notAuthorised().url
      )
    }
    ...
    "redirect to invitationAlreadyResponded when an invitation is returned that is already actioned" in {
      givenInvitationAccepted(arn, mtdItId.value, invitationIdITSA, serviceITSA, identifierITSA)
      val resultITSA = getConfirmInvitationITSA(
        authorisedAsValidClientITSA(FakeRequest().withSession("agencyName" -> "My Agency"), mtdItId.value))
    ...
    "redirect to /incorrect/ if authenticated user has HMRC-MTD-IT enrolment but with a different mtdItId.value" in {
      givenInvitationNoPermission(mtdItId.value, invitationIdITSA, identifierITSA)
      val resultITSA = getConfirmInvitationITSA(
        authorisedAsValidClientITSA(FakeRequest().withSession("agencyName" -> "My Agency"), mtdItId.value))
  ...
  "POST /accept-tax-agent-invitation/confirm-decline/:invitationId (confirm decline page)" should {
    ...
    "redirect to /client/not-authorised if an authenticated user has a Confidence Level below 200" in {
      givenUnauthorisedForInsufficientConfidenceLevel()
      val request = FakeRequest("POST", routes.ClientsInvitationController.submitConfirmDecline(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.submitConfirmDecline(invitationIdITSA)(
        authenticatedClient(
          request,
  ...
  "GET /reject-tax-agent-invitation/declined/:invitationId" should {
    ...
    "redirect to invitationAlreadyResponded when declined a invitation that is already actioned" in {
      givenInvitationExists(arn, mtdItId.value, invitationIdITSA, serviceITSA, identifierITSA, "Declined")
    ...
    "redirect to IV uplift journey if an authenticated user has a Confidence Level below 200" in {
      givenUnauthorisedForInsufficientConfidenceLevel()
      val request = FakeRequest("GET", routes.ClientsInvitationController.getInvitationDeclined(invitationIdITSA).url)
        .withSession("agencyName" -> "My Agency")
      val result = controller.getInvitationDeclined(invitationIdITSA)(
        authenticatedClient(
          request,
      ...
      checkRedirectedToIVUplift(
        result,
        expectedCompletionUrl = routes.ClientsInvitationController.getInvitationDeclined(invitationIdITSA).url,
        expectedFailureUrl = routes.ClientsInvitationController.notAuthorised().url
      )
    }
  }
  ...
  "GET /decide-later/:invitationId" should {
    val getDecideLaterITSA = controller.getDecideLater(invitationIdITSA)
    ...
    "show the decide later page with ITSA content even when user is not authenticated" in {
      ...
      checkHtmlResultWithBodyText(result, htmlEscapedMessage("decide-later.itsa.p1"))
      checkHtmlResultWithBodyText(result, htmlEscapedMessage("decide-later.subheader1"))
      await(bodyOf(result)) should not include htmlEscapedMessage("common.sign-out")
    }

    "show a signout url on the landing page if the user is authenticated" in {
      val result = getDecideLaterITSA(FakeRequest().withCookies(Cookie("mdtp", "authToken=Bearer+")))
      status(result) shouldBe OK
      checkHasClientSignOutUrl(result)
    }
  }
