...
class AgentInvitationsVATControllerJourneyISpec extends BaseISpec with AuthBehaviours {
  ...
  "POST /agents/select-service" should {
    ...
    "return 303 for authorised Agent with valid VAT service when there is nothing in the cache, redirect to client-type" in {
      val serviceForm = ServiceTypeForm.form.fill(serviceVAT)
      val result =
        submitService(authorisedAsValidAgent(request.withFormUrlEncodedBody(serviceForm.data.toSeq: _*), arn.value))
  ...
  "POST /agents/select-service-business" should {
    val request = FakeRequest("POST", "/agents/select-service-business")
    val submitServiceBusiness = controller.submitSelectServiceBusiness()

    "return 303 for an authorised Agent when YES is selects, redirect to identify-client" in {
      testCurrentAuthorisationRequestCache.save(CurrentAuthorisationRequest(business, serviceVAT))
      val confirmForm = agentConfirmationForm("error").fill(Confirmation(true))

      val result = submitServiceBusiness(authorisedAsValidAgent(request.withFormUrlEncodedBody(confirmForm.data.toSeq: _*), arn.value))
    ...
    "return 303 for an authorised Agent when NO is selected and there is nothing in the cache, redirect to select-client-type" in {
      val confirmForm = agentConfirmationForm("error").fill(Confirmation(false))
    ...
    "redisplay the page with errors when neither YES or NO are selected" in {
      testCurrentAuthorisationRequestCache.save(CurrentAuthorisationRequest(business, serviceVAT))

      val result = submitServiceBusiness(authorisedAsValidAgent(request, arn.value))

      status(result) shouldBe 200
      checkHtmlResultWithBodyMsgs(result, "error.business-service.required")
    }
